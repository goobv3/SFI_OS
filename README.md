...
### 📌 v1.4.1 - 2026.03 (Hotfix) 🛠️
**"API 라우팅 최적화 및 DB 영속성 이슈 해결"**
- **API 라우팅 오류 및 405 Method Not Allowed 해결:** 중복된 라우트 정의를 통합하고 레거시 경로(`/api/houses`)와 신규 경로(`/api/metadata/houses`)를 모두 지원하는 하이브리드 라우팅 레이어 구축.
- **백엔드 컴파일 에러(Linker Error) 수정:** `WeatherManager`의 미구현 메서드(`recordFarmWeather`)를 구현하여 빌드 안정성 확보.
- **데이터베이스 영속성(Persistence) 안정화:** 하우스 및 센서 설정 정보가 유실되지 않고 DB에 정상 저장되도록 백엔드 로직 동기화 완료.
