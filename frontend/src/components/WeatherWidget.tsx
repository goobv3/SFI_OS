...
const WeatherWidget: React.FC<WeatherWidgetProps> = ({ compact = false }) => {
    ...
    const getTitle = (isKma: boolean) => {
        if (!isKma) return t('farmWeather');
        if (hoursAhead === 0) return t('kmaWeather');
        return t('forecastTemplate').replace('{h}', hoursAhead.toString());
    };
...
// --- [분리된 날씨 카드 컴포넌트] ---
const WeatherCard = ({ 
    title, data, isKma = false, hoursAhead, setHoursAhead, t, getDisplayTime, compact 
}: { 
    title: string, data: WeatherInfo | null, isKma?: boolean, 
    hoursAhead: number, setHoursAhead: (h: number) => void, t: any, 
    getDisplayTime: () => string, compact: boolean 
}) => {
    if (!data) {
        return (
